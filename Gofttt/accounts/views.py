...
class UserLoginView(View):
    ...
    def post(self , request):
        ...
        user = authenticate(username=phone_number, password=password, email=email)
        if user is not None:
            login(request, user)
    ...
    def post(self, request):
        ...
        if user_input == code_instance.otp:
            ...
            if now < temp:
                ...
                user = User.objects.create_user(phone_number=user_session['phone_number'], email=user_session['email'], full_name=user_session['username'], display_name=display_name1, password=user_session['password'])
                user.save()
...
class ForgotPasswordNewView(View):
    def get(self, request):
        try:
            ...
            return render(request, "accounts/reset_password2.html", context)
        except:
            messages.error(request, "خطا دوباره تلاش کنید", 'danger')
            return redirect("home:index")
    ...
    def post(self, request):
        ...
        if password != "":
            ...
            if email == customer.email:
                ...
                messages.error(request, "ایمیل وارد شده اشتباه است", 'danger')
                return redirect("accounts:new_password")   
        else:
            del request.session['0']
            messages.error(request, "خطا دوباره تلاش کنید", 'danger')
